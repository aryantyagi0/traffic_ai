...
def collect_credentials(targets, app_name):
    ...
    if "aws" in targets:
        creds["aws"] = {
            ...
            "region":            get_value("AWS_REGION",            "AWS_REGION"),
            "instance_role_arn": os.getenv("AWS_INSTANCE_ROLE_ARN", ""),
            "app_name":          app_name,
            "fork_url":          "",
        }
    if "azure" in targets:
...
def deploy_to_aws(folder, creds):
    ...
    env_vars = {"PORT": "8080"}
    env_vars.update(creds.get("env_vars", {}))
    runtime_env = [{"Name": k, "Value": v} for k, v in env_vars.items()]
    ...
    try:
        resp = apprunner.create_service(
            ...
            SourceConfiguration={
                "CodeRepository": {
                    "RepositoryUrl": f"https://github.com/{repo_path}",
                    "SourceCodeVersion": {"Type": "BRANCH", "Value": "ai-docker-setup"},
            ...
            InstanceConfiguration={
                "Cpu":    "1 vCPU",
                "Memory": "2 GB",
                "InstanceRoleArn": creds.get("instance_role_arn", ""),
            },
        )
        url = f"https://{resp['Service']['ServiceUrl']}"
        print(f"[AWS] ✅ App Runner deploying from GitHub branch ai-docker-setup")
        print(f"[AWS] ✅ {url}")
        return url
    except Exception as e:
        print(f"[AWS] Error: {e}")
